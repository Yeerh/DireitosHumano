...
const RetroEffect = forwardRef<Effect, RetroEffectProps>(({ colorNum, pixelSize }, ref) => {
  // WrappedRetro é um componente React gerado pelo wrapEffect
  // O ref aqui é apenas para compatibilidade; não é obrigatório no seu uso.
...
function DitheredWaves({
  ...
  const prevColor = useRef<[number, number, number]>([...waveColor] as [number, number, number]);
  ...
  useFrame(({ clock }) => {
    ...
    if (!prevColor.current.every((v, i) => v === waveColor[i])) {
      ...
      prevColor.current = [...waveColor] as [number, number, number];
    }
  ...
  return (
    <>
      ...
      <mesh
        // @react-three/fiber tipa esse handler como (event) => void; usamos `any` para compatibilidade.
